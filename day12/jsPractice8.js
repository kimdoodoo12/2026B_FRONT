...
function productAdd(pcode){

    // 1. 입력받은 값
    let category = document.querySelector('.category').value
    let name = document.querySelector('.name').value
    let price = document.querySelector('.price').value
    let image = document.querySelector('.image').files[0] // + 이미지
    let pdate = `${new Date().getFullYear()}-${new Date().getMonth()}-${new Date().getDate()}`

    if (category == 'disabled'){ alert("카테고리를 선택해주세요"); return}

    let object = {ccode: category, pname: name, pprice: price, pimg: image == undefined ? 'https://placehold.co/100' : URL.createObjectURL(image), pdate: pdate , pcode: finalPcode++}
