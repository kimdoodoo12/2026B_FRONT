...
function renderProducts(products) {
    for (i in products){
        document.getElementById("productName").innerHTML += `${products[i].name}  `
        document.getElementById("productPrice").innerHTML += `${products[i].price }원 `
        document.getElementById("productStock").innerHTML += `${products[i].stock }개 `
    }
}
...
function renderList(array){
    for (i in array){
        document.getElementsByClassName("toDoList")[0].innerHTML += `<li>${array[i]}</li>`
    }
}
...
function getTicketPrice(age) {
    if (age < 8) return "무료"
    else if (age >= 8 && age <20) return "5000원"
    else if (age >= 20) return "10000원"
    else return
}
